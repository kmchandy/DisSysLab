...
- **`calendar_stylist`** can use **`web_scraper`** on **NOAA `forecast.weather.gov` (MapClick)** or **`weatherapi`** on **WeatherAPI.com**. For **`weatherapi`**, set **`WEATHERAPI_KEY`**; **`office_compiler`** prefetches JSON and appends a date table to every role’s prompt. For **`web_scraper`** with a `forecast.weather.gov` URL, the **custom app backend** prefetches the same HTML before **Run** and sets **`OFFICE_WEATHERAPI_DIGEST`** with a **period** table so agents match weekdays like the situation room (`dsl run` from the shell skips that prefetch unless you set that env yourself). Install backend deps so **`beautifulsoup4`** is available for the NOAA prefetch.
