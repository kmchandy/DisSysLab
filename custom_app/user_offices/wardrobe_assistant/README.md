...
`dsl run` (and thus the Custom App **Run**) calls `dissyslab.office_v2.office_run_context.apply_office_run_context_to_environ(...)`, merging optional env payloads into Claude system prompts (`OFFICE_*_DIGEST` keys). **`python build/run.py` skips this** unless you export those vars yourself — prefer **`dsl run`**.
