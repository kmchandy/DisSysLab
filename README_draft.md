...
receives messages. The network specifies how messages flow between agents. Offices are
...
newer), and nothing else, to get started. You do not need to know
Python: every step below is a sentence you tell Cowork, and it does the
typing.
...
*"add the weather for Pasadena too"*, or *"only tell me about the
urgent ones"*.

What you end up with is a folder on your computer holding the app and
whatever it produces — an HTML page, a file of records. It runs when you
start it and keeps going until you stop it, so an app set to check every
hour goes on checking. Closing Cowork does not stop it; shutting the
laptop does, and it resumes from where it stopped when you start it
again.
...
with the library. There are about forty. If you want to build an
