...
class Network(Block):
    ...
    def connect_ports(self):
        ...
        for from_block, from_port, to_block, to_port in self.connections:
            try:
                ...
                raise ValueError(
                    f"Invalid block name {e} in connection {from_block}.{from_port}->{to_block}.{to_port}"
                )
    ...
    def connect(self):
        ...
        try:
            ...
            raise RuntimeError(
                f"""Error while making connections in network '{self.name}'.
                block names are {self.blocks.keys()}
                Connections are {self.blocks.items()}
    Likely causes:
    • A port is not declared or misspelled.
    • A block name in a connection is incorrect.
    • An inport or outport is missing.
    . If you are using queues (and not messages) check inport queues.
