...
outside the unit quarter-circle (``x² + y² >= 1``), increments an
outside counter and emits a ``{"kind": "outside", "running_count": N}``
message. If the point is inside, emits nothing.
...
class _OutsideClassifier(Agent):
    ...
    def run(self):
        while True:
            ...
            if x * x + y * y >= 1.0:
                self.count += 1
                self.send(
                    {"kind": "outside", "running_count": self.count},
                    "out_",
                )


role = AgentRoleEntry(
    name="outside_classifier",
    in_ports=("in_",),
    out_ports=("out",),
    factory=_OutsideClassifier,
)
