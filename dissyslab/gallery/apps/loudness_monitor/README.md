...
(`samples/thunderstorm.wav`) so the demo works on a fresh clone
with no audio equipment. The only dependency is numpy:

```bash
pip install numpy
```

That is it. WAV decoding uses the Python standard library; no
librosa, no numba, no llvmlite, no native audio codec. Then:
...
```bash
# macOS
afplay dissyslab/gallery/apps/loudness_monitor/samples/thunderstorm.wav

# Linux
aplay dissyslab/gallery/apps/loudness_monitor/samples/thunderstorm.wav
```
...
MIT. The synthetic `thunderstorm.wav` is generated by
`make_thunder.py` and is released under the same MIT license as the
rest of the framework. The optional dependencies `sounddevice`
(PortAudio binding, only used for live-mic capture via `audio_mic`)
and `librosa` (only used for non-WAV file decoding) carry their own
licenses; consult their projects.
