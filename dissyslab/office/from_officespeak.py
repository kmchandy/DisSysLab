...
def _write_python_role(roles_dir: Path, role_name: str, agent: AgentSpec, fn_factory) -> None:
    ...
    # If assemble.py renamed this agent's outbox (Track A's original name,
    # e.g. "alert", forced to "out"), let the approved code keep returning
    # that original name -- Role accepts either, via status_aliases.
    aliases = dict(agent.status_aliases)
    aliases_kwarg = f", status_aliases={aliases!r}" if aliases else ""
    ...
    file_text = (
        ...
        f"    factory=lambda: Role(fn={fn_factory.__name__}(), statuses={list(statuses)!r}{aliases_kwarg}),\n"
        f")\n"
    )
