...
def to_list(target: list, key: Optional[str] = None) -> Callable[[Any], None]:
    """
...
def to_set(target_set: set, key: Optional[str] = None) -> Callable[[Any, Any], None]:
    def _fn(msg):
        target_set.add(_maybe_extract(msg, key))
    return _fn


def to_file(path: str, key: Optional[str] = None) -> Callable[[Any, Any], None]:
    def _fn(msg):
...
def to_jsonl(path: str, key: Optional[str] = None) -> Callable[[Any, Any], None]:
    def _fn(msg):
        val = _maybe_extract(msg, key)
        with open(path, "a", encoding="utf-8") as f:
